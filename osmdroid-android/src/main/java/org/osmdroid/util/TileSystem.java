...
abstract public class TileSystem {
     ...
     * 瓦片投影方式
     */
    protected static GeoConstants.ProjectionType mProjectionType = GeoConstants.ProjectionType.WEB_MERCATOR;
    ...
    public static double getTileSize(final double pZoomLevel) {
        return MapSizeX(pZoomLevel - getInputTileZoomLevel(pZoomLevel));
    }
     ...
     * @since 6.0.0
     */
    public static double MapSizeX(final double pZoomLevel) {
        return getTileSize() * getFactor(pZoomLevel);
    }
     ...
     * 区分不同投影坐标系的瓦片行列号数量
     */
    public static double MapSizeY(final double pZoomLevel) {
        switch (mProjectionType) {
            case PLATTE_CARRE:
                return getTileSize() * getFactor(pZoomLevel) / 2;
            case WEB_MERCATOR:
            default:
                return getTileSize() * getFactor(pZoomLevel);
    ...
    public static double GroundResolution(final double latitude, final double zoomLevel) {
        return GroundResolutionMapSize(wrap(latitude, -90, 90, 180), MapSizeX(zoomLevel));
    }
    ...
    public GeoPoint PixelXYToLatLong(
        ...
        return getGeoFromMercator(pixelX, pixelY, MapSizeX(zoomLevel), reuse, true, true);
    }
    ...
    public GeoPoint PixelXYToLatLongWithoutWrap(
            final int pixelX, final int pixelY, final double zoomLevel, final GeoPoint reuse) {
        final double mapSize = MapSizeX(zoomLevel);
...
//		Log.d("xzw", "getGeoFromMercator: "+out.getLatitude()+":"+out.getLongitude());
    ...
    public GeoConstants.ProjectionType getmProjectionType() {
        return mProjectionType;
    }

    public void setmProjectionType(GeoConstants.ProjectionType mProjectionType) {
